...
const semesterDrop = document.querySelector(".semester-dropdown");
const semesterDropButton = document.querySelector(".dropdown-button");
...
function updateReportCard(reportCardTableElement, currentSemester) {
  ...
  const semesterList = studentData[currentSemester];
  semesterList.forEach((course, i) => addCourseRowToReportCard(reportCardTableElement, course, i));
  //for(let i = 0; i < studentData.currentSemester.length; i++)
  //{
  //  addCourseRowToReportCard(reportCardTableElement, studentData[currentSemester][i], i);
  //}
...
function addEventListeners(
  ...
  fallSemesterElement.addEventListener("click", updateReportCard(fallSemesterElement), closeDropdown(dropdownElement));
  springSemesterElement.addEventListener("click", updateReportCard(springSemesterElement), closeDropdown(dropdownElement));
  winterTermElement.addEventListener("click", updateReportCard(winterTermElement), closeDropdown(dropdownElement));
}
...
function addUpStudentCredits(reportCardTableElement) {
  let total = 0;
  let lettersList = reportCardTableElement.querySelectorAll(".credit");
  lettersList.forEach(item => (total += parseInt(item.innerHTML)))
  return total;
}
